// This is the host-side benchmark application that runs on the ARM CPU and communicates with the FPGA via XRT.
// It provides both a software reference implementation for comparison and orchestrates the hardware kernel execution,
// measuring performance (throughput, latency) and validating correctness against golden reference outputs.
...
int main(int argc, char **argv)
{
    ...
    hog_sw(img, sw_features); // warm-up
    ...
    krnl(bo_image, bo_feat).wait(); // warm-up
    ...
    {
        auto t0 = std::chrono::high_resolution_clock::now();
        bo_image.sync(XCL_BO_SYNC_BO_TO_DEVICE); // H2D: image in
        auto t1 = std::chrono::high_resolution_clock::now();
        ...
        bo_feat.sync(XCL_BO_SYNC_BO_FROM_DEVICE); // D2H: features out
